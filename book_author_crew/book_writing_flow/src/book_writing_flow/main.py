...
from book_writing_flow.crews.write_book_chapter_crew.write_book_chapter_crew import (
    WriteBookChapterCrew,
)
from book_writing_flow.types import Chapter, ChapterOutline

from .crews.outline_book_crew.outline_crew import OutlineCrew

class BookState(BaseModel):
    title: str = (
        "The Current State of AI in November of 2024 - Trends Across the Industry"
    )
    book: List[Chapter] = []
    book_outline: List[ChapterOutline] = []
    topic: str = (
        "Exploring the latest trends in AI across different industries as of November 2024"
    )
    goal: str = """
...
class BookFlow(Flow[BookState]):
    ...
    async def join_and_save_chapter(self):
        ...
        # Create the filename by replacing spaces with the underscores and adding .md extension
